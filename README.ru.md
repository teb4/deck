...
- [Интеграция с AgentTeams](#интеграция-с-agentteams)
...
### Интеграция с AgentTeams

Deck Editor можно использовать как **навык (Skill)** в рамках фреймворка [AgentTeams](https://github.com/agentscope-ai/AgentTeams). Это предоставляет агентам безопасный и транзакционный способ совместного редактирования файлов.

#### Как это работает в качестве навыка

*   **Готовый к импорту навык**: Deck Editor предварительно упакован как навык для AgentTeams. Определение навыка (включая его имя, описание и использование) находится в каталоге `.claude/skills/deck-editor/` этого репозитория.
*   **Атомарные операции**: Навык использует ключевой механизм Deck (`REV` и атомарный `APPLY`), чтобы гарантировать, что изменения файлов применяются без повреждений и конфликтов версий.
*   **Координация нескольких агентов**: Навык выступает в роли общего ресурса для команды агентов. Агенты могут читать файлы (получая `REV`) и отправлять изменения. Если файл был изменен другим агентом, навык отклонит устаревшее редактирование, вынудив агента перечитать файл.

#### Пример: Команда агентов-разработчиков

В типичном сценарии разработки вы можете создать команду AgentTeams со следующими агентами:
1.  **Агент-разработчик**: Использует навык `deck-editor` для написания и изменения кода.
2.  **Агент-ревьюер**: Использует навык для чтения файлов и проверки изменений.
3.  **Агент-тестировщик**: Использует навык для чтения результатов тестов и логов.

Рабочий процесс для исправления ошибки будет выглядеть так:
1.  **Агент-ревьюер** идентифицирует ошибку и читает `src/main.py`, используя навык, запоминая текущий `REV`.
2.  **Агенту-разработчику** поручается исправление. Он снова читает файл (получая тот же `REV`), планирует изменения и **применяет** их.
3.  Если за это время другой агент изменил `src/main.py`, применение изменений (`apply`) завершится ошибкой, и агент-разработчик будет уведомлен о необходимости перечитать файл.

Это обеспечивает надежную среду для совместной работы без конфликтов.
