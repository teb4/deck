...
Для тех, у кого нет времени читать все.
Для чего это (очень коротко):

  1. Чтобы LLM не мучилась, безуспешно пытаясь изменить файл с помощью sed или diff/patch, а потом говорила: "Не могу изменить, перепишу полностью"
  2. Чтобы модель могла обнаружить, что файл уже кто-то другой изменил, пока она раздумывала
  3. Чтобы не испортить файл, если вдруг модель по какой-то причине забыла дописать несколько строк в конце файла.
...
#### Unicode, эмодзи и иероглифы

`REPLACE_REGEX` работает с полным Unicode — эмодзи, иероглифы, латинские с диакритикой и любые другие символы, поддерживаемые движком Python `re`.

```text
# Заменить конкретный эмодзи в строке с несколькими эмодзи
# Вход: 🔥🎉✨🚀💥
@APPLY a3f5b7c9d1e2f405
@REPLACE_REGEX 1
s/🎉/party/g
...
s/测试/test/g
@END
# Результат: testtesttest

# Заменить символы с диакритикой в нескольких строках
@APPLY a3f5b7c9d1e2f405
@REPLACE_REGEX 1-10
s/é/e/g
@END
# café → cafe, résumé → resume (все совпадения в строках 1–10)

# Использовать другой разделитель, чтобы избежать конфликтов
@APPLY a3f5b7c9d1e2f405
@REPLACE_REGEX 1
s|🔥|fire|g
@END
...
# Заменить сложный эмодзи (составной grapheme-кластер с ZWNJ)
# Вход: Hello 👨‍👩‍👧‍👦 world
@APPLY a3f5b7c9d1e2f405
@REPLACE_REGEX 1
s/👨‍👩‍👧‍👦/family/g
@END
# Результат: Hello family world
```
